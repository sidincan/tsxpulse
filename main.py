...
def compute_signal(stock: dict, idx: int = 0, total: int = 0) -> dict:
    ...
    try:
        ...
        # IMS — fetch previous session closing candles (last 90 min) for intraday validation
        # Only run for stocks scoring >= 0.55 base CMS to avoid wasting time on weak signals
            ...
            try:
                ...
                if not raw_intra.empty:
                    ...
                    if prev_dates:
                        ...
                        # Last 30 min: 3:30 PM - 4:00 PM ET
                        # Institutional algorithms execute final positioning here — highest signal quality
                        last90 = day_df[(day_df.index.hour > 15) |
                                        ((day_df.index.hour == 15) & (day_df.index.minute >= 30))]
                        if len(last90) >= 2:  # 30-min window yields fewer candles
