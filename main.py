...
def compute_signal(stock: dict, idx: int = 0, total: int = 0) -> dict:
    ...
    try:
        ...
        # Pre-market / after-hours / weekend: use previous session last 30 min
        ims_score  = 0.5  # neutral default if fetch fails
        ims_trend  = "NEUTRAL"
        ims_detail = {}
        if cms_base >= 0.55:
            try:
                ...
                if not raw_intra.empty:
                    ...
                    if market_is_open:
                        # ── LIVE SESSION: all candles from 9:30 AM today ──────
                    ...
                    if not market_is_open:
                        ...
                        if not prev_dates:
                            raise ValueError("No previous session data available")
                        prev_date = prev_dates[-1]
                        day_df = raw_intra[raw_intra.index.strftime("%Y-%m-%d") == prev_date]
                        window = day_df[(day_df.index.hour > 15) |
                                        ((day_df.index.hour == 15) & (day_df.index.minute >= 30))]
                        ims_session_label = "PREV_CLOSE"

                    if len(window) >= 2:
                        c = window["Close"].squeeze().astype(float)
                        v = window["Volume"].squeeze().astype(float)
                        o = window["Open"].squeeze().astype(float)
                        h = window["High"].squeeze().astype(float)
                        l = window["Low"].squeeze().astype(float)
                        typical = (h + l + c) / 3
                        ...
                        red_c   = int(((c - o) < 0).sum())
                        # IMS scoring — same weights regardless of session
                        ims_score = 0.0
                        ...
                        # Label carries session context for frontend display
                        if ims_session_label == "LIVE":
                            ims_trend = "LIVE_UP"   if ims_score >= 0.65 else                                         "LIVE_DOWN" if ims_score <= 0.30 else "LIVE_NEUTRAL"
                        else:
                            ims_trend = "STRONG_CLOSE" if ims_score >= 0.65 else                                         "WEAK_CLOSE"   if ims_score <= 0.30 else "NEUTRAL"
                        ims_detail = {
        ...
        # Detect overnight gap risk before flagging as entry signal.
        # If the stock is down >3% from scan price in pre/post-market, flag it.
        # This prevents ABBV/AA-style situations where signal price was ~$226
        # but stock opened at $196 due to overnight earnings event.
        gap_risk     = False
        gap_pct      = None
        premarket_px = None
        if entry_signal:
            try:
                ...
                if not pm_data.empty:
                    ...
                    if "Close" in pm_data.columns:
                        ...
                        # >3% gap down from scan price = GAP RISK flag
                        if raw_gap < -3.0:
                            gap_risk = True
                            entry_signal = False  # suppress entry — untradeable at signal price
...
def sequential_scan(universe: list) -> list:
    ...
    for idx, stock in enumerate(universe, 1):
        ...
        time.sleep(0.5)   # 500ms between requests — prevents rate limiting (increased from 0.3s)
...
def rescore_ticker(ticker: str, market: str = "TSX"):
    """Re-score a single tracked ticker using the same CMS logic as the full scan.
    Used by the tracker to detect stale/expired signals on D3+ positions."""
    try:
        stock = {
            "ticker":   ticker,
            "market":   market,
...
def get_candles(ticker: str, market: str = "TSX", interval: str = "5m", session: str = "today"):
    ...
    session=previous → last 90 min of previous session (for pre-market scan validation)
    ...
    try:
        ...
        # Download 2 days of intraday data to ensure we have yesterday + today
        raw = yf.download(
            yf_ticker,
            period="2d",
            interval=interval,
        ...
        if session == "today":
            # Only today's candles from 9:30 AM onwards
            df = raw[raw.index.strftime("%Y-%m-%d") == today_str]
            df = df[df.index.hour >= 9]
        else:
            # Previous session: last 90 min (2:30 PM - 4:00 PM ET of most recent completed day)
            # Find the most recent trading day that isn't today
            all_dates = sorted(set(raw.index.strftime("%Y-%m-%d")))
            prev_dates = [d for d in all_dates if d < today_str]
            if not prev_dates:
                # Weekend/holiday edge case — use earliest available
                prev_dates = [all_dates[0]]
            prev_date = prev_dates[-1]
            day_df = raw[raw.index.strftime("%Y-%m-%d") == prev_date]
            # Last 30 minutes: 15:30 to 16:00 ET — institutional final positioning window
            df = day_df[(day_df.index.hour > 15) |
                        ((day_df.index.hour == 15) & (day_df.index.minute >= 30))]

        if df.empty or len(df) < 2:  # 30-min window yields fewer candles
            return {
        ...
        vol_trend = "increasing" if vol_second_half > vol_first_half * 1.1 else                     "decreasing" if vol_second_half < vol_first_half * 0.9 else "steady"

        # Green vs red candle count and volume weighting
        ...
        # Close strength — where did price close relative to its range?
        ...
        # Higher highs / lower lows check (last 4 candles)
        if len(high) >= 4:
        ...
        # IMS score for pre-market scan validation (0.0 - 1.0)
        ims_score = 0.0
        ...
        return {
            ...
            "metrics": {
                ...
                "session":          "LIVE" if session == "today" else "PREV_CLOSE"
            }
        }
...
def track_signal(ticker: str, signal_date: str, market: str = "TSX", entry: float = None):
    try:
        yf_ticker = f"{ticker}.TO" if market == "TSX" else ticker
        ...
        # ── BOUNDED DATE FETCH — root cause fix for back-fill bug ─────────────
        # Using period="1y" then slicing from signal_date forward caused all 15
        # day rows to populate with the same date when signal was near today.
        # Fix: fetch only the exact 15-day window from signal_date → signal_date+25bd
        # 25 calendar days covers 15 trading days with weekends + holidays
        ...
        # Slice from signal date forward, cap at 15 trading days
        ...
        for i, (date, row) in enumerate(df_after.iterrows()):
            # Force scalar — .squeeze() prevents Series/float() error (TD.TO bug)
            def _s(v):
            ...
            days.append({
                ...
                "high":   round(h, 2),
                "low":    round(l, 2),
                "midday": round((h + l) / 2, 2),
            })

        first_close = days[0]["close"] if days else "N/A"

        # ── DATA COLLISION GUARD ─────────────────────────────────────────────
        # Detects yfinance contamination: if a concurrent scan download returns
        # another stock's data (e.g. BMO.TO price for ABBV /track call).
        # If entry price was passed and D1 close differs by >25%, reject.
        # 25% is generous enough for volatile stocks but catches yfinance collisions
        # where a completely different ticker's price bleeds in (typically 30-60% off)
        if entry and days:
            deviation = abs(days[0]["close"] - entry) / entry
            if deviation > 0.25:
                print(f"DATA COLLISION DETECTED {ticker}: entry=${entry:.2f} but D1=${days[0]['close']:.2f} ({deviation*100:.0f}% diff) — rejecting contaminated data")
                return {"error": f"Data collision detected for {ticker} — retry in a moment"}
...
def get_headlines(ticker: str, market: str = "TSX"):
    """
    Fetch news headlines server-side — no CORS issues.
    TSX: Yahoo Finance RSS (free, solid Canadian coverage)
    NYSE: Finnhub company-news API
    Falls back to empty list on any error.
    """
    headlines = []
    try:
        if market == "TSX":
            # Yahoo Finance RSS for TSX stocks
            yf_ticker = ticker + ".TO"
            rss_url   = f"https://feeds.finance.yahoo.com/rss/2.0/headline?s={yf_ticker}&region=CA&lang=en-CA"
            ...
            if resp.status_code == 200 and resp.text.strip():
                ...
                for item in items:
                    ...
                    try:
                        ...
                        t = parsedate_to_datetime(pubdate).strftime("%I:%M %p") if pubdate else ""
                    except Exception:
                        t = ""
                    if title:
                        headlines.append({"headline": title, "source": source, "time": t})
            # Fallback: try yfinance .news if RSS empty
            if not headlines:
                print(f"HEADLINES: Yahoo RSS empty for {ticker}, trying yfinance.news")
                info = yf.Ticker(yf_ticker).news or []
                for n in info[:8]:
                        ...
                        for n in data[:8]:
                            ...
                            if hl:
                                headlines.append({
                                    "headline": hl,
                                    "source":   n.get("source", ""),
                                    "time":     datetime.fromtimestamp(n.get("datetime", 0)).strftime("%I:%M %p") if n.get("datetime") else ""
                                })
    except Exception as e:
        print(f"HEADLINES ERROR {ticker}: {e}")

    return {"ticker": ticker, "market": market, "headlines": headlines, "count": len(headlines)}
