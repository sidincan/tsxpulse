...
def compute_signal(stock: dict, idx: int = 0, total: int = 0) -> dict:
    ...
    try:
            ...
            try:
                ...
                if not raw_intra.empty:
                    ...
                    if prev_dates:
                        ...
                        # Last 90 min: 2:30 PM - 4:00 PM ET
                        last90 = day_df[(day_df.index.hour > 14) |
        ...
        # Final CMS: 4 base components (90%) + IMS (10%)
        cms = round(cms_base * 0.90 + ims_score * 0.10, 3)
...
def get_candles(ticker: str, market: str = "TSX", interval: str = "5m", session: str = "today"):
    ...
    try:
        ...
        if session == "today":
            ...
            # Last 90 minutes: 14:30 to 16:00 ET
            df = day_df[(day_df.index.hour > 14) |
                        ((day_df.index.hour == 14) & (day_df.index.minute >= 30))]
